...
var templates = { // each function will return a object filled with user varables that will represent the email template
  newApp: function(user ,appInfo) { // called in newSend fn below
    return {
  ...
  closedApp: function(user, appInfo) { // called by closedSend fn below
    return {
      from: '"AppTrak" <' + emailConfig.email_user + '>',
      to: user.username + ' <' + emailConfig.email_user + '>',
  ...
  weeklyReminder: function(username ,userEmail, numberOfApps) {
    return { // called by send fn below
      from: '"AppTrak" <' + emailConfig.email_user + '>',
  ...
  deletedApp: function(user, appInfo) { // takes user and appInfo object
    return { // called by deletedSend fn below
      from: '"AppTrak" <' + emailConfig.email_user + '>', 
      to: user.username + ' <' +  emailConfig.email_user + '>',
...
var email = {
  send: function(user, userApps) { // fn is called by weeklyReminder fn in emailHelpers.js
    ...
    transporter.sendMail(options, function(error, info) { // call transporter.sendMail with options and callback
      if(error) {
  ...
  newSend: function(req, res) {
    var user;
    Users.find({_id:req.body.data.userId}, function(err, result) { // search for user by userId passed in req
      ...
      transporter.sendMail(options, function(err, info) { // calls transporter.sendMail built-in fn passing in options and callback
  ...
  closedSend: function(req, res) {
    var user;
    Users.find({_id:req.body.data.userId}, function(err, results) { // search for userId passed in req
